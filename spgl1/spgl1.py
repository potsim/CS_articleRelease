...
##TODO the place to force the model order
def _oneprojector_i(b, tau):
    ...
    if tau >= bNorm:
        # print('Oh nonnnnnnnnn')
        # x = b.copy()
        # m_order = 16
        # if np.sum(x >= 0) >= m_order :
        #     ind = np.argsort(x)[::-1]
        #     x[ind[m_order:]] = 0
        # return x
        return b.copy()
...
def _norm_l1_project(x, weights, tau):
    ...
    if np.all(np.isreal(x)):
        ...
        # print('    real -- order before and after {}, {}'.format( np.sum(x != 0.0), np.sum(xproj != 0.0)))
    else:
        xa = np.abs(x)
        idx = xa < _eps
        ...
        xc[idx] = 0
        xproj = x * xc
        # print('    complex -- order before, after, at the end {}, {}, {}'.format( np.sum(xa != 0.0),
                    # np.sum(xc != 0.0), np.sum(xproj != 0.0)))
    return xproj
...
def _spg_line_curvy(x, g, fmax, A, b, project, weights, tau):
    ...
    while 1:
        ...
        # print('  Inside _spg_line_curvy order: {}'.format(np.sum(xnew != 0.0)))
...
def spgl1(A, b, tau=0, sigma=0, x0=None, fid=None, verbosity=0,
          ...
          dual_norm=_norm_l1_dual, m_ord_limit=None):
    r"""SPGL1 solver.
    ...
    """
    #SPO patch to limit model order
    global m_order
    m_order = m_ord_limit
    # m_order = None
    ...
    if tau == 0:
        ...
        single_tau = True
        single_tau = False #SPO modif to start from a known point
        # print("SPO modif to start from a know point")
    ...
    while 1:
        ...
        if stat:
            # # test to limit m_order here
            # m_order = m_ord_limit
            # print('Enter LAST  spg_line_curvy .............')
            # f, x, r, niter_line, stepg, lnerr, \
            # time_project_curvy, time_matprod_curvy = \
            #    _spg_line_curvy(x, gstep*g, max(last_fv), A, b,
            #                    project, weights, tau)

            # print('Out  spg_line_curvy ....Model order: {}'.format(np.sum(x != 0.0)))
            break
        ...
        while 1:
            ...
            if lnerr:
                ...
                print('Projected backtrack failed. Retry with feasible dirn linesearch.')
                x = xold.copy()
                f = fold
                start_time_project = time.time()
